...
var retailProductDB = {
        ...
        return new Promise((resolve, reject) => {
            ...
            conn.connect(function(err) {
                if (err) {
                    ...
                    const sql = `SELECT i.ID as id, i.NAME as name, rp.IMAGEURL as imageURL, i.SKU as sku, i.DESCRIPTION as description,
                                i.TYPE as type, i.CATEGORY as category, rp.FAVORITES as favorites
                    ...
                    conn.query(sql, function(err, result) {
                        if (err) {
                            ...
                            const productList = result.map(row => {
                                const retailProduct = new RetailProduct();
                                retailProduct.id = row.id;
                                retailProduct.name = row.name;
                                retailProduct.imageURL = row.imageURL;
                                retailProduct.sku = row.sku;
                                retailProduct.description = row.description;
                                retailProduct.type = row.type;
                                retailProduct.category = row.category;
                                retailProduct.favorites = row.favorites;
    ...
    updateFavoriteStatus: function(productId, isFavorite) {
        return new Promise((resolve, reject) => {
            const conn = db.getConnection();
            conn.connect(function(err) {
                if (err) {
                    ...
                    const sql = `UPDATE retailproductentity SET FAVORITES = ? WHERE ID = ?`;
                    conn.query(sql, [isFavorite, productId], function(err, result) {
                        if (err) {
                            conn.end();
                            return reject(err);
                        } else {
